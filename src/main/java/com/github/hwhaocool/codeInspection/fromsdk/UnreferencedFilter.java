...
public class UnreferencedFilter extends RefUnreachableFilter {
    ...
    public int getElementProblemCount(@NotNull RefJavaElement refElement) {
        ...
        if (refElement.isEntry() || !((RefElementImpl) refElement).isSuspicious() || refElement.isSyntheticJSP()) {
            return 0;
        }
        ...
        if (!((GlobalInspectionContextBase) myContext).isToCheckMember(refElement, myTool)) {
            return 0;
        }
